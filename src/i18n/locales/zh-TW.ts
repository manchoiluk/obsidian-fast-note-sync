...
export default {
  ...
  "setting.sync.hash_limit_toggle_desc": "開啟後將限制單次同步週期內允許計算雜湊的文件數量，以防止在大庫全量掃描時導致 Obsidian 記憶體溢出（OOM）崩潰。關閉後則不限制計算數量。",
  "setting.sync.hash_limit": "雜湊計算數量限制",
  "setting.sync.hash_limit_desc": "單次同步週期内最大允許計算雜湊的文件數。超出限制的文件將在後續的同步週期中繼續處理。預設值為 20000。",
